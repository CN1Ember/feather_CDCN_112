                ...
                if i == 0:
                    if self.avgdown and s != 1:
                        downsample = nn.Sequential(nn.AvgPool2d(2, stride=2),
                                                   ...
                                                   nn.Conv2d(input_channel, output_channe+l, kernel_size=1, bias=False))
...
def FaceFeatherNetA_PWS(se=False, width_mult=1.0):
    model = FaceFeatherNet_PWS(se = se, width_mult=width_mult)
    return model

def FaceFeatherNetB_PWS(se=True, img_channel=1, width_mult=1.0):
    model = FaceFeatherNet_PWS(se=se, avgdown=True, img_channel=img_channel, width_mult=width_mult)
    return model
