...
import roc
import models
from read_data.read_data_for_pws import CASIA
from losses import *
...
parser.add_argument('--arch', '-a', metavar='ARCH', default='FaceFeatherNetA_PWS',
                    choices=model_names,
                    help='models architecture: ' +
                         ' | '.join(model_names) +
                         ' (default: resnet18)')
parser.add_argument('--e_arch', '-ea', metavar='ARCH', default='PixelMapEstimator',
                    choices=model_names,
                    help='models architecture: ' +
                         ' | '.join(model_names) +
                         ' (default: resnet18)')
parser.add_argument('--config', default='cfgs/blank.yaml')
...
parser.add_argument('--gpus', type=str, default='4,5', help='use gpus training eg.--gups 0,1')
...
parser.add_argument('--phase_val', default=False, type=bool,
...
parser.add_argument('--mean', default=0.23589583, type=float, help='mean for training data')
parser.add_argument('--std', default=0.22553962, type=float, help='std for training data')
parser.add_argument('--lambda_cls', default=1, type=float, help='coefficient for cls loss')
parser.add_argument('--lambda_pws', default=1, type=float, help='coefficient for pws loss')
parser.add_argument('--save_path', type=str, default='checkpoints/Default_save_path')
parser.add_argument('--input_channel', type=int, default=32, help='input channel of estimator')
parser.add_argument('--out_channel', type=int, default=2, help='output channel of estimator')
parser.add_argument('--face_like', type=bool, default=False, help='whether use face-like mask')
...
logger = Logger('./logs_visualization/logs_{}'.format(args.save_path.split('/')[1]))
...
def main():
    ...
    with open('logs/model_config/{}_{}.txt'.format(time_stp, args.save_path.split('/')[1]), 'w') as config_log:
        config_dict = args.__dict__
        for key in config_dict:
            if '__' not in key:
                config_log.write(str(key) + ": " + str(config_dict[key]) + "\n")
    ...
    estimator = models.__dict__[args.e_arch](in_chn=args.input_channel, num_class=args.out_channel)
    ...
    if USE_GPU:
        ...
        estimator = torch.nn.DataParallel(estimator, device_ids=args.gpus)
        model.to(device)
        estimator.to(device)
    ...
    criterion = {
        'focal': FocalLoss(device, 2, gamma=args.fl_gamma),
        'softmax': nn.CrossEntropyLoss().to(device)
    }

    optimizer_dict = [
        {"params": filter(lambda p: p.requires_grad, model.parameters()), "lr": args.lr},
        {"params": filter(lambda p: p.requires_grad, estimator.parameters()), "lr": args.lr},
    ]
    optimizer = torch.optim.SGD(optimizer_dict, args.lr, momentum=args.momentum,
    ...
    # normalize = transforms.Normalize(mean=[0.21735254], std=[0.21561144])  # chenguo
    ...
    val_dataset = CASIA(transforms.Compose([
        ...
        transforms.ToTensor(),
        normalize,
    ]), phase_train=False, phase_val = args.phase_val, phase_test=args.phase_test, face_like=args.face_like)
    ...
    # valid before train
    prec1 = validate(val_loader, model, estimator, criterion, -1)

    for epoch in range(args.start_epoch, args.epochs):
        adjust_learning_rate(optimizer, epoch)
        # train for one epoch
        train(train_loader, model, estimator, criterion, optimizer, epoch)
        # evaluate on validation set
        prec1 = validate(val_loader, model, estimator, criterion, epoch)
        ...
        save_name = '{}/{}_{}_{:.3f}_best.pth.tar'.format(args.save_path, args.model_name, epoch, prec1) if is_best else \
            '{}/{}_{}_{:.3f}_.pth.tar'.format(args.save_path, args.model_name, epoch, prec1)
        ...
        if is_best or epoch>=args.epochs-1:
            save_estimator(epoch=epoch+1, model=estimator, filename=save_name.replace(".pth.tar", "_estimator.pth.tar"))


def train(train_loader, model, estimator, criterion, optimizer, epoch):
    batch_time = AverageMeter()
    data_time = AverageMeter()
    cls_losses = AverageMeter()
    pws_losses = AverageMeter()
    ...
    for i, (input, target, target_fmap) in enumerate(train_loader):
        # measure data loading time
        # print("line284:",input.shape)
        data_time.update(time.time() - end)
        input_var = Variable(input).float().to(device)
        target_var = Variable(target).long().to(device)
        target_fmap_var = Variable(target_fmap).long().to(device)

        # compute output
        output, feature = model(input_var)
        fmap_output = estimator(feature)
        cls_loss = criterion['focal'](output, target_var)
        pws_loss = criterion['softmax'](fmap_output, target_fmap_var)
        total_loss = args.lambda_cls * cls_loss + args.lambda_pws * pws_loss
        ...
        reduced_cls_loss = cls_loss.data.clone()
        cls_losses.update(reduced_cls_loss)
        reduced_pws_loss = pws_loss.data.clone()
        pws_losses.update(reduced_pws_loss)
        reduced_total_loss = total_loss.data.clone()
        ...
        if i % args.print_freq == 0:
            with open('logs/{}_{}.log'.format(time_stp, args.arch), 'a+') as flog:
                line = 'Epoch: [{0}][{1}/{2}]\t lr:{3:.5f}\t' \
                       ...
                       'PWS_Loss {pws_loss.val:.4f} ({pws_loss.avg:.4f})\t' \
                       'Prec@1 {top1.val:.3f} ({top1.avg:.3f})\t' \
                    .format(epoch, i, len(train_loader), lr,
                            batch_time=batch_time, cls_loss=cls_losses, pws_loss=pws_losses, top1=top1)
                print(line)
                flog.write('{}\n'.format(line))
    # =================== Log =============================
    info = {'train_cls_loss': cls_losses.avg, 'train_pws_loss': pws_losses.avg, 'train_total_loss': total_losses.avg, 'train_accuracy': top1.avg, 'learning_rate': lr}
    for tag, value in info.items():
        logger.scalar_summary(tag, value, epoch)


def validate(val_loader, model, estimator, criterion, epoch):
    global time_stp
    batch_time = AverageMeter()
    cls_losses = AverageMeter()
    pws_losses = AverageMeter()
    ...
    with torch.no_grad():
        for i, (input, target, target_fmap) in enumerate(val_loader):
            with torch.no_grad():
                input_var = Variable(input).float().to(device)
                target_var = Variable(target).long().to(device)
                target_fmap_var = Variable(target_fmap).long().to(device)
                ...
                # measure accuracy and record loss
                prec1, prec2 = accuracy(output.data, target_var, topk=(1, 2))
                cls_losses.update(cls_loss.data, input.size(0))
                pws_losses.update(pws_loss.data, input.size(0))
                total_losses.update(total_loss.data, input.size(0))
                ...
                # print(soft_output[0][0])
                # print(soft_output)
                # print("check line377")
                # preds = soft_output.to(device).detach().numpy()
                preds = soft_output.cpu()
                ...
                if i % args.print_freq == 0:
                    line = 'Test: [{0}/{1}]\t' \
                           ...
                           i, len(val_loader), batch_time=batch_time,cls_loss=cls_losses, pws_loss=pws_losses, top1=top1)
    ...
    # =================== Log =============================
    info = {'valid_cls_loss': cls_losses.avg, 'valid_pws_loss': pws_losses.avg, 'valid_total_loss': total_losses.avg, 'valid_accuracy': top1.avg}
    for tag, value in info.items():
        logger.scalar_summary(tag, value, epoch)
...
def save_estimator(epoch, model, filename='checkpoint.pth.tar'):
    check_point_params = {}
    if isinstance(model, nn.DataParallel):
        check_point_params["model"] = model.module.state_dict()
    else:
